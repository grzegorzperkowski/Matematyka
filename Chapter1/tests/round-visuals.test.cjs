...
test("group and number-line visuals carry explicit mathematical data", () => {
  const eightByEight = config.buildQuestions("multdiv").find((question) => question.prompt.includes("8 · 8"));
  assert.deepEqual(JSON.parse(JSON.stringify(eightByEight.visual)), { type: "array", groups: 8, itemsPerGroup: 8, caption: "8 równych grup po 8 elementów." });
