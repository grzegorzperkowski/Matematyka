(() => {
  ...
  const pair = (shape, leftNumerator, leftDenominator, rightNumerator, rightDenominator, caption) =>
    model(shape, leftNumerator, leftDenominator, caption, { compare: { numerator: rightNumerator, denominator: rightDenominator } });
  ...
  function comparisonQuestions() {
    ...
    return [
      choice("<", ["<", ">", "="], { label: "Wspólny mianownik", prompt: `Wstaw znak: ${rawFraction(small, denominator)} ? ${rawFraction(large, denominator)}`, hint: "Przy jednakowych mianownikach porównaj liczniki.", explanation: `${small} < ${large}, więc ${rawFraction(small, denominator)} < ${rawFraction(large, denominator)}.`, visual: pair("bar", small, denominator, large, denominator, "Obie całości podzielono na tyle samo równych części.") }),
      choice(">", ["<", ">", "="], { label: "Wspólny licznik", prompt: `Wstaw znak: ${rawFraction(numerator, smallDenominator)} ? ${rawFraction(numerator, largeDenominator)}`, hint: "Przy jednakowych licznikach większe części powstają przy mniejszym mianowniku.", explanation: `${smallDenominator} < ${largeDenominator}, więc ${rawFraction(numerator, smallDenominator)} > ${rawFraction(numerator, largeDenominator)}.`, visual: pair("bar", numerator, smallDenominator, numerator, largeDenominator, "Ta sama liczba części: mniejszy mianownik daje większe kawałki.") }),
      choice("<", ["<", ">", "="], { label: "Liczby mieszane", prompt: `Wstaw znak: ${whole} ${rawFraction(partA, mixedDenominator)} ? ${whole} ${rawFraction(partB, mixedDenominator)}`, hint: "Części całkowite są równe, więc porównaj części ułamkowe.", explanation: `${partA} < ${partB}, więc pierwsza liczba mieszana jest mniejsza.`, visual: pair("circle", whole * mixedDenominator + partA, mixedDenominator, whole * mixedDenominator + partB, mixedDenominator, "Najpierw porównaj pełne koła, potem pozostałe części.") }),
      choice("<", ["<", ">", "="], { label: "Porównanie z połową", prompt: `Wstaw znak: ${rawFraction(belowHalf, halfDenominator)} ? 1/2`, hint: "Połowa mianownika byłaby licznikiem ułamka równego 1/2.", explanation: `Połowa ${halfDenominator} to ${halfDenominator / 2}, a ${belowHalf} jest mniejsze, więc ułamek jest mniejszy od 1/2.`, visual: pair("bar", belowHalf, halfDenominator, halfDenominator / 2, halfDenominator, "Porównaj zaznaczenie z połową paska.") }),
      choice(">", ["<", ">", "="], { label: "Porównanie z jednością", prompt: `Wstaw znak: ${rawFraction(denominator + 2, denominator)} ? 1`, hint: "Porównaj licznik z mianownikiem.", explanation: "Licznik jest większy od mianownika, więc ułamek jest większy od 1.", visual: pair("bar", denominator + 2, denominator, denominator, denominator, "Pełny pasek po prawej to jedna całość.") }),
      choice(`${rawFraction(small, denominator)}, ${rawFraction(large, denominator)}`, [`${rawFraction(small, denominator)}, ${rawFraction(large, denominator)}`, `${rawFraction(large, denominator)}, ${rawFraction(small, denominator)}`], { label: "Porządkowanie", prompt: "Który zapis ustawia ułamki od mniejszego do większego?", hint: "Mianowniki są jednakowe, więc rosnący licznik oznacza rosnący ułamek.", explanation: `${small} < ${large}, więc poprawna jest kolejność ${rawFraction(small, denominator)}, ${rawFraction(large, denominator)}.`, visual: pair("bar", small, denominator, large, denominator, "Większe zaznaczenie stoi dalej na prawo.") }),
      choice(rawFraction(denominator - 1, denominator), [rawFraction(denominator - 1, denominator), rawFraction(denominator - 3, denominator)], { label: "Blisko jedności", prompt: "Który ułamek leży bliżej liczby 1?", hint: "Sprawdź, ilu części brakuje każdemu ułamkowi do całej jednostki.", explanation: `${rawFraction(denominator - 1, denominator)} brakuje tylko ${rawFraction(1, denominator)} do 1.`, visual: pair("bar", denominator - 1, denominator, denominator - 3, denominator, "Mniejsza jasna luka do pełna oznacza bliższą liczbę.") }),
      choice("=", ["<", ">", "="], { label: "Równe ułamki", prompt: "Wstaw znak: 2/3 ? 8/12", hint: "Rozszerz 2/3 przez 4.", explanation: "2/3 = 8/12, bo licznik i mianownik pomnożono przez 4.", visual: pair("bar", 2, 3, 8, 12, "Różny podział może zakrywać tę samą część całości.") }),
      choice("Maja", ["Maja", "Olek", "oboje tyle samo"], { label: "Zadanie tekstowe", prompt: `Maja przeszła ${rawFraction(large, denominator)} trasy, a Olek ${rawFraction(small, denominator)}. Kto przeszedł większą część?`, hint: "Porównaj liczniki, bo mianowniki są równe.", explanation: `${large} > ${small}, więc większą część trasy przeszła Maja.`, visual: pair("bar", large, denominator, small, denominator, "Obie trasy podzielono na tyle samo równych odcinków.") }),
      choice(">", ["<", ">", "="], { label: "Wspólny mianownik", prompt: "Wstaw znak: 3/4 ? 5/8", hint: "Rozszerz 3/4 do mianownika 8.", explanation: "3/4 = 6/8, a 6/8 > 5/8, więc 3/4 > 5/8.", visual: pair("bar", 3, 4, 5, 8, "Sprowadź oba paski do jednakowego rozmiaru części.") })
    ];
  }
